...
class SignUp_Form extends Component{
    ...
    onSubmit = (e) => {
        ...
        .catch(error => {
            this.setState({serverErrors:error.response.data.errors.errors})
        })
    ...
    const {errors,serverErrors, data, loading, saved} = this.state;
        console.log(errors);
            return( 
              ...
              <div>
                ...
                <Well>
                 ...
                 {serverErrors.username && <Alert bsStyle="danger">
                    <h4>Oh, You Got an Error</h4>
                   {Object.keys(serverErrors).map((key) => 
                        <p key={key}>{serverErrors[key].message}</p>                                 
                    ) }
                 </Alert> }
